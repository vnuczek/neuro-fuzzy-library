...
void ksi::CV::split(const ksi::dataset dataset, const int n)
{
    datasets.clear();
    datasets.resize(n);
...
void ksi::CV::save(const std::filesystem::path& directory) const
{
    ...
    for (auto i = 0; i < datasets.size(); ++i)
    {
        ...
        {
            throw std::runtime_error("Unable to open file " + (directory / ("dataset_" + std::to_string(i) + ".txt")).string());
        }
    }
}

ksi::dataset ksi::CV::read_file(const std::filesystem::path& file_directory)
{
	return pReader->read(file_directory.string());
}

void ksi::CV::read_directory(const std::filesystem::path& directory)
{
